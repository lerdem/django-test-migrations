...
from django_test_migrations.db.checks import check_statement_timeout_setting
...
def test_correct_statement_timeout(mocker, connection_mock_factory, vendor):
    ...
    mocker.patch(
        'django_test_migrations.db.checks.connections.all',
...
def test_multiple_connections(mocker, connection_mock_factory):
    ...
    mocker.patch(
        'django_test_migrations.db.checks.connections.all',
        return_value=connections_mocks,
    )
    check_messages = check_statement_timeout_setting()
    assert len(check_messages) == 2
